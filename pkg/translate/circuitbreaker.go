...
// package's circuit breaker defaults (see WithCircuitBreaker): five
// consecutive full-retry-exhaustion failures trip the breaker, and it
// stays open for 10s before letting a single probe call through. This
// avoids paying the full retry-backoff latency budget on every request
// during a sustained GPT-4o outage, once the outage is already confirmed
// by several consecutive fully-exhausted calls. An isolated flaky call
// (one attempt fails but a retry within the same Translate call still
// succeeds) never trips the breaker; only several consecutive full
// exhaustions do.
const (
...
// *consecutive full-retry-exhaustion* failures -- calls where every
// attempt in Translate's retry loop failed with a transient error -- not
// every individual HTTP attempt, and not permanent (4xx) failures, which
// already fail fast and never touch the breaker.
//
